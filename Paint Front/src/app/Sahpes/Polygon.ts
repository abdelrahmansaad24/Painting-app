...
export class Polygon implements Shape{
  ...
  constructor(private render:Renderer2, private SVG:any){this.type = 'square';}



  draw(event: MouseEvent): void {
    this.width =  Math.min(Math.abs(event.offsetX - this.x1), Math.abs(event.offsetY - this.y1));
